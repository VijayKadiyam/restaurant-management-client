      ...
      <v-flex xs12 sm8 md8>
        ...
        <v-card class="elevation-12"> 
          ...
          <v-card-title primary-title>
            <h3 class="headline mb-0">Edit a new menu item
...
<script type="text/javascript">
  ...
  export default {
    ...
    mounted() {
      ...
      this.form.get(`/api/product-categories/${this.$route.params.menu_item_id}`)
        .then(data  =>  {
          this.form = new Form({
            'name'  : data.data.name,
            'menu_category_id' : parseInt(data.data.menu_category_id),
    ...
    methods: {
      ...
      update() {
        this.form.patch(`/api/product-categories/${this.$route.params.menu_item_id}`)
          .then(data  =>  {
